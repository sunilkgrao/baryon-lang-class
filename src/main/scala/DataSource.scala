...
class DataSource (
  ...
  private def readEventData(sc: SparkContext) : RDD[Observation] = {
    ...
    PEventStore.find(
      ...
      eventNames = Some(List("sentence")) // specify data event name
      ...
      Observation(
        if (label == "english") {1.0}
        else if (label == "french") {2.0}
        else if (label == "spanish") {3.0}
        else if (label == "german") {4.0}
        else 0.0,
  ...
  Seq[(TrainingData, EmptyEvaluationInfo, RDD[(Query, ActualResult)])] = {
    ...
    (0 until dsp.evalK.get).map { k =>
      ...
      val test = data.filter(_._2 % dsp.evalK.get == k)
        .map(_._1)
        .map(e => (new Query(e.text), new ActualResult(e.language)))
